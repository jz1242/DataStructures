...
/**Jason Zhang jzhan127 Assignment6
...
public final class Angelina {
    ...
    private static void solveJolie() {
        ...
        Queue<Vertex<String>> list = new LinkedList<>();
        Boolean back = false;
        LinkedList<Vertex<String>> toPrint = new LinkedList<>();
        ArrayList<Vertex<String>> visited = new ArrayList<>();
        Vertex<String> a = actor;
        list.add(jolie);
        visited.add(jolie);
        int count = 0;
        boolean keepPrint = false;
        while (list.size() != 0 && !back) {
            Vertex<String> temp = list.remove();
            if (temp.equals(actor)) {
                back = true;
                keepPrint = true;
            }
            else {
                for (Edge<String> n : graph.outgoing(temp)) {
                    if (!(visited.contains(graph.to(n)))) {
                        graph.label(graph.to(n), temp);
                        list.add(graph.to(n));
                        visited.add(graph.to(n));
                    }
                }
            }
        }
        while (keepPrint) {
            ...
            if (graph.label(a) != null) {
                a = (Vertex<String>) graph.label(a);
            }
            else{
                keepPrint = false;
            }
